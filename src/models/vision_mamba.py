# from: https://github.com/OliverRensu/ARM/blob/main/Finetuning/models_mamba.py
import torch
...
import math
from .layers.mamba_simple import Mamba
...
class ARM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if if_abs_pos_embed:
            trunc_normal_(self.pos_embed.pos_emb, std=0.02)
        if if_cls_token:
            if use_double_cls_token:
    ...
    def forward_features(
        ...
        self.patch_embed.permute = False
        x = self.patch_embed(data, perm)
        B, M, _ = x.shape
        ...
        # get pos embed
        pos_seq = (
            torch.arange(M + 1, dtype=torch.long, device=x.device)
            .unsqueeze(0)
            .expand(B, -1)
        )
        ...
        if self.global_pool:
            return (
                hidden_states[:, :token_position, :].mean(1)
                + hidden_states[:, token_position + 1 :, :].mean(1)
            ) / 2.0
        else:
