...
class Mamba(nn.Module):
    def __init__(
        ...
        if bimamba_type == "v1":
            ...
            self.dt_proj_c = nn.Linear(
                self.dt_rank, self.d_inner, bias=True, **factory_kwargs
            )

            self.D_c = nn.Parameter(
                torch.ones(self.d_inner, device=device)
            )  # Keep in fp32
            ...
            self.dt_proj_c_b = nn.Linear(
                self.dt_rank, self.d_inner, bias=True, **factory_kwargs
            )

            self.D_c_b = nn.Parameter(
                torch.ones(self.d_inner, device=device)
            )  # Keep in fp32
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if (
            ...
            if self.bimamba_type == "v1":
                ...
                token_position = L // 2
                cls, xc = (
                    xz[:, :, token_position : token_position + 1],
                    torch.cat(
                        [xz[:, :, :token_position], xz[:, :, token_position + 1 :]],
                        dim=-1,
                    ),
                )
                ...
                xc = torch.cat(
                    (xc[:, :, :token_position], cls, xc[:, :, token_position:]), dim=-1
                )
                ...
                cls, out_c = (
                    out_c[:, :, token_position : token_position + 1],
                    torch.cat(
                        [
                            out_c[:, :, :token_position],
                            out_c[:, :, token_position + 1 :],
                        ],
                        dim=-1,
                    ),
                )
                ...
                out_c = torch.cat(
                    (out_c[:, :, :token_position], cls, out_c[:, :, token_position:]),
                    dim=-1,
                )
                out = out + out_b.flip([-1])
                out = F.linear(
                    rearrange((out + out_c) / 4.0, "b d l -> b l d"),
                    self.out_proj.weight,
                    self.out_proj.bias,
                )
